...
for i=1:length(nSTOs)
    ...
    for iter=1:MaxIter
        ...
        StartPoint=1;
        SearchLen=2*Ng;
        %[STO_dif_temp2,mag_dif_temp2]= SymbolSTOlcp(y_aw,Ng,StartPoint,SearchLen); % bigdot works!
        [sto_pos, FFTDataIn,SymbolData,sto_diffsum]=SymbolSTOlcp(y_aw,Ng,StartPoint,SearchLen); % bigdot works!
        %[sto_pos, sto_diffsum]=SymbolSTOlcp(y,Ng,StartPoint,SearchLen,Nfft)
        %%%%%经验证，以上两者函数结果一致
        figure;
        plot(mag_dif_temp,'.')
        hold on
